...
export class DependencyChecker {
    ...
    private static async getMissingComponents(missing: string[], migrationObjects: MigrationTool[]): Promise<DependencyReport[]> {
        const missingReports: DependencyReport[] = [];

        for (const dep of missing) {
            const component = migrationObjects.find(obj => obj.getName() === dep);
            const report = component ? await this.createReportForFoundComponent(component, dep) : await this.createReportForUnknownComponent(dep);
