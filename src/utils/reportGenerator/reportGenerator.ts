...
export function generateHtmlTable<T>(
          ...
          ${columns
            .map((col) => {
              ...
              return row[key] !== undefined
                ? `<td ${dataAttr} title="${title}" key="${key}" value="${value}">${cellContent}</td>`
                : '';
            })
            .join('')}
        </tr>
  ...
  return `
    ...
    <div class="table-container">
      ...
      <script src="../src/utils/reportGenerator/reportGeneratorUtility.js" defer></script>
      <link rel="stylesheet" href="../src/utils/reportGenerator/reportGenerator.css">
    </div>
  `;
}
