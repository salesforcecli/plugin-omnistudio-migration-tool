...
export default class Assess extends OmniStudioBaseCommand {
  ...
  protected static flagsConfig = {
    ...
    allversions: flags.boolean({
      char: 'a',
      description: MessageService.getMessage('assessAllVersionsDescription'),
      required: false,
    }),
    ...
    verbose: flags.builtin({
      type: 'builtin',
      description: MessageService.getMessage('enableVerboseOutput'),
    }),
  };
  ...
  public async run(): Promise<any> {
    ...
    try {
      ...
      Logger.error(MessageService.getMessage('errorRunningAssess'), error);
      process.exit(1);
    }
  }
    ...
    if (relatedObjects) {
      ...
      for (const obj of objectsToProcess) {
        if (!validOptions.includes(obj)) {
          Logger.error(MessageService.getMessage('invalidRelatedObjectsOption', [String(obj)]));
          process.exit(1);
        }
      }
